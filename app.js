...
// ========== DATA ==========
    const techs = [
      ...
      },
      {
        ...
        date: "4/28/2026",
        scores: {
          // Self-eval submitted 4/28/26 (10-row sections averaged into 4-item buckets)
          electrical: { low_voltage: 4, high_voltage: 5, schematics: 5, open_circuit: 4 },
          heating: { furnace_sequence: 5, components: 5, heat_exchanger: 4, gas_flue: 4 },
          cooling: { superheat_subcool: 4, refrigerant_charge: 4, refrigerant_cycle: 4, vacuum_reclaim: 4 },
          airflow: { static_pressure: 3, airflow_troubleshoot: 4, duct_awareness: 4, iaq: 4 },
          install: { blower_fan: 5, evap_coil: 5, tstat_zoning: 5, compressor: 5 },
          customer: { communication: 4, explaining: 3, solutions: 3, closing: 2 },
          advanced: { communicating_sys: 4, dual_fuel: 4, zone_board: 2, iaq_dehumid: 4 },
        ...
        strengths: ["Outgoing & kind", "Organizational skills", "Installation", "Customer relationship building"],
        weaknesses: ["Objection handling experience", "Gets too technical with customers", "Providing solutions during maintenance calls"],
        growth: "Technical skills, troubleshooting, and customer sales skills",
        training: "Sales/service training, NATE certification, intermittent issues training",
        holding_back: "Lack of experience in sales/service",
        managerNotes: "Day 1: 4/28/2026. New service technician with ~3 years prior HVAC experience. Aptitude test 4/28: 35/50 (70%) \u2014 Service Technician level. Self-eval 4/28 (signed): rates self strongest in Install (5.0/5) and Electrical (4.5/5), lowest in Customer/Sales (3.0/5). Most confident: low-volt shorts, safeties, high-voltage troubleshoot. Least confident: intermittent control board issues, offering personalized upgrades. EPA certified. Watch for self-rating gap vs reality \u2014 Cooling self=4.0 but aptitude=5/10 (50%); revisit after first 30 days.",
        managerTags: [
    ...
    // Dewone is the only Saturday tech. Source: IMG_0211 (Nexstar daily) + IMG_0212 (Memberships).
    //   Dewone: $572 svc, 100% conv (2/2), $252 avg, 1 SPP, 0 TGL, 3.6 sold hrs, 2 tasks/call, 1 mem sold / 2 mem opps (50%).
    // No installs.
    // v210: Bumped flag v1\u2192v2 to correct mem opps from 1\u21922 (IMG_0212). v1-aware guard so we add only the +1 opp delta if v1 already fired.
    function _wlbSeedDay20260502IfNeeded() {
      try {
        var FLAG = 'snappy_wlb_day_seeded_2026_05_02_v2';
        if (localStorage.getItem(FLAG) === '1') return;
        var v1Already = localStorage.getItem('snappy_wlb_day_seeded_2026_05_02_v1') === '1';
        var d = _wlbLoad();
        var WEEK = '2026-04-27';
        var existing = d[WEEK] || {};
        function add(short, svc, ic, ir, ms, mo) {
        ...
        if (v1Already) {
          // v1 already added svc 572, mem 1/1. Only the +1 mem opp delta is missing.
          add('Dewone', 0, 0, 0, 0, 1);
        } else {
          add('Dewone', 572, 0, 0, 1, 2);
        }
        d[WEEK] = existing;
        _wlbSave(d);
        try { localStorage.setItem(WEEKLY_VIEW_KEY, WEEK); } catch(e) {}
    ...
    // Daily 4/28 per tech:
    //   Daniel:  $1,287 svc, 0 installs, 0 mem opps
    //   Benji:   $444 svc, 1 install $9,059.59, 1 mem opp (no sale)
    //   Chris:   $275 svc, 0 installs, 2 mem opps (no sale)
    //   Dee:     $0 svc, 0 installs, 0 mem opps
    //   Dewone:  $0 svc, 0 installs, 0 mem opps
    function _wlbSeedDay20260428IfNeeded() {
      try {
        var FLAG = 'snappy_wlb_day_seeded_2026_04_28_v1';
        if (localStorage.getItem(FLAG) === '1') return;
        var d = _wlbLoad();
        var WEEK = '2026-04-27';
        var existing = d[WEEK] || {};
        ...
        add('Daniel', 1287, 0, 0,        0, 0);
        add('Benji',   444, 1, 9059.59,  0, 1);
        add('Chris',   275, 0, 0,        0, 2);
        // Dee, Dewone, Nick had $0 across — nothing to add
        d[WEEK] = existing;
        _wlbSave(d);
        try { localStorage.setItem(WEEKLY_VIEW_KEY, WEEK); } catch(e) {}
        localStorage.setItem(FLAG, '1');
      } catch(e) { console.warn('_wlbSeedDay20260428IfNeeded failed', e); }
    }
    ...
    // v159 (Approach 2): Performance as a PURE ADDITIVE BONUS — only helps, never hurts.
    //
    // Bonus tiers (based on best-of-recent leaderboard total points):
    //   30+ pts/wk → +3.0 composite pts  (S week)
    //   20–29 pts/wk → +2.0 composite pts  (A week)
    //   10–19 pts/wk → +1.0 composite pts  (B week)
    //   1–9 pts/wk  → +0.5 composite pts  (any activity)
    //   0 pts or no data → +0.0  (no penalty, ever)
    //
    // Basis: prefers THIS WEEK's total. If no data this week, uses the HIGHEST of the
    // last 4 prior weeks (so a recent strong week still rewards). This way, performance
    // pulls composite up but a slow week or missing data never pulls it down.
    //
    // Returns { bonus, basis: 'thisWeek'|'recentBest'|'none', basisPts, basisWeek, label, hasData }
    function calcPerformanceBonus(tech) {
      try {
        if (typeof _wlbLoad !== 'function' || typeof _wlbWeekStart !== 'function' || typeof _wlbPrevWeek !== 'function' || typeof _wlbReadEntry !== 'function' || typeof _wlbPoints !== 'function') {
          return { bonus: 0, basis: 'none', basisPts: 0, basisWeek: null, label: 'No data', hasData: false };
        }
        var data = _wlbLoad();
    ...
    function getTechTier(tech) {
      ...
      // 9. Performance Bonus (v159, Approach 2): pure additive bonus from leaderboard — only helps, never hurts
      const perfBonusData = calcPerformanceBonus(tech);
